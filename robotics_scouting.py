...
def main():
    ...
    with col1:
        ...
        match_number = st.number_input("Match Number", step=1)
    ...
    with col4:
        ...
        end_position = st.selectbox(
            "End Position",
            ["No Climb", "Failed Climb", "Parked", "Onstage", "Spotlight"],
        )
        harmony = st.selectbox(
            "Harmony", ["First on chain", "Second on chain", "Third on chain"]
        )

    with col5:
        st.subheader("Postmatch")
        offensive_skill = st.selectbox(
            "Offensive Skill",
            ["Not effective", "Average", "Very Effective", "Not Observed"],
        )
        defensive_skill = st.selectbox(
            "Defensive Skill",
            ["Not effective", "Average", "Very Effective", "Not Observed"],
        )
        died = st.toggle("Died")
